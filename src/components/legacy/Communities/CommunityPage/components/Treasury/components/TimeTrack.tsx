...
import { HistoryIcon, useStyles } from "../TreasuryStyle";
...
const TimeTrack = ({ endTime, theme, ...props }: TimeTrackProps) => {
  ...
  React.useEffect(() => {
    ...
    const timerId = setInterval(() => {
      ...
      let delta = Math.floor(
        endTime ? endTime.getTime() - now.getTime() : (addADayDate.getTime() - now.getTime()) / 1000
      ); // diff in secs
  ...
  return (
    <Box
      className={cls({ [classes.timeCardDark]: theme === "dark" }, classes.timeCard)}
      display="flex"
      flexDirection="row"
      alignItems="center"
