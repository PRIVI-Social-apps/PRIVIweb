  ...
  useEffect(() => {
    ...
    if (props.community) {
      ...
      if(members && members.length > 0) {
        let indexMemberV2 = members.findIndex(
          (address) => address === userSelector.address
        );
        if (indexMemberV2 !== -1) {
          isInCommunity = true;
        }
      ...
      if(treasurers && treasurers.length > 0) {
        let indexTreasurerV2 = treasurers.findIndex(
          (address) => address === userSelector.address
        );
      ...
      if(founders && founders.length > 0) {
        let indexFounderV2 = founders.findIndex(
          (address) => address === userSelector.address
        );
        if (indexFounderV2 !== -1) {
          isInCommunity = true;
      ...
      if(props.community.Admins && props.community.Admins.length > 0) {
        let indexAdmin = props.community.Admins.findIndex(
          (admin) => admin.userId === userSelector.id
        );
        if (indexAdmin !== -1) {
          isInCommunity = true;
        }
      ...
      if(props.community.Members && props.community.Members.length > 0) {
        let indexMember = props.community.Members.findIndex(
          (member) => member.id === userSelector.id
        );
        if (indexMember !== -1) {
          isInCommunity = true;
        }
      ...
      if(props.community.UserRoles) {
        ...
        for (let userRole of userRolesKeys) {
          if (
            props.community.UserRoles[userRole] &&
            props.community.UserRoles[userRole].role
          ) {
            let rolesArray = Object.keys(
              props.community.UserRoles[userRole].role
            );
  ...
  return (
    <div className="jarr-container">
      {userIsInCommunity ? (
        ...
        <div className="joinCommunityLabel">
          Join Community to see Community Jarr
        </div>
      )}
    </div>
  );
