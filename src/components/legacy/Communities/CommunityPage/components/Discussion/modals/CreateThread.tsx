...
import { Grid, Tooltip, InputAdornment, IconButton, Fade, Input } from "@material-ui/core";
...
export default function CreateThread(props: any) {
  ...
  const uploadThreadDescriptionImages = async id => {
    return new Promise((resolve, reject) => {
      const formData = new FormData();
      photos.forEach((photo, i) => {
        formData.append("image", photo, "photo" + i);
  ...
  const HashtagLabel = p => {
    ...
    return (
      <Box className={classes.hashtagLabel}>
  ...
  return (
    <Modal size="medium" isOpen={props.open} onClose={props.handleClose} showCloseIcon>
      <div className={classes.modalContant}>
        <div className={classes.titleCommunitiesModal}>Create new thread</div>
        ...
        >
          <Grid xs={12} md={6}>
            <Box className={classes.flexRowInputs}>
              <Box className={classes.infoHeaderCreatePod}>
                Hash tags
                <Tooltip
            ...
            <Box className={classes.hashtagsRowCreatePod} style={{ height: "auto", margin: "10px 0 0 0" }}>
              ...
              {hashTags && hashTags.length ? (
                <Box className={classes.flexRowInputs} style={{ padding: "5px 0" }}>
        ...
        <Post title="Thread description" editor={editor} setterEditor={setEditor} />
        ...
        <Box display="flex" justifyContent="flex-end">
          <LoadingWrapper loading={creationProgress}>
            <PrimaryButton size="medium" onClick={createThread}>
              Post Thread
            </PrimaryButton>
          </LoadingWrapper>
        </Box>
