      ...
      <div className={classes.proposalsContent}>
        {/*proposals && proposals.length > 0 ? (
          <div className={classes.artistsList}>
            <div className={classes.appbarContainer}>
              <AppBar position="static" className={classes.appbar}>
                <Tabs
                  ...
                  value={proposalsTabSelction}
                  className={classes.tabs}
                  onChange={(e, value) => {
                    setProposalsTabSelection(value);
                    if (proposalIndexSelection !== -1 && proposals[proposalIndexSelection]) {
                      setProposalsTabSelection(-1);
                    }
                  }}
                >
                  {["Active", "Declined history"].map((name, index) => (
                    <Tab
                      className={cls({ [classes.selected]: index === proposalsTabSelction }, classes.tab)}
                      label={name}
                      key={name}
                    />
                  ))}
            ...
            {proposalIndexSelection !== -1 && proposals[proposalIndexSelection] ? (
              <div className={classes.artistTiles}>
                <img
                  className={classes.close}
                  src={require("assets/icons/cross_gray.png")}
                  alt="close"
                  onClick={() => {
                    setProposalIndexSelection(-1);
                  }}
                />
                <div className={classes.proposalHeader}>
                  {proposals[proposalIndexSelection].date && getDate(proposals[proposalIndexSelection].date)}
                  <Box display={"flex"} alignItems="center">
                    <span>
                      {proposals[proposalIndexSelection].date &&
                        new Date(proposals[proposalIndexSelection].date).getTime() > new Date().getTime() &&
                        getStyledTime(new Date(), proposals[proposalIndexSelection].date, false)}{" "}
                      left
                    </span>
                    <img src={require("assets/icons/clock_pink.png")} alt="clock" />
                  </Box>
                </div>

                {proposals[proposalIndexSelection].artists &&
                  proposals[proposalIndexSelection].artists.length > 0 &&
                  proposals[proposalIndexSelection].artists.map((artist, index) => (
                    <ArtistTile key={`artist-${index}`} artist={artist} pod={pod} />
                  ))}
                {proposals[proposalIndexSelection].status.toUpperCase() !== "DECLINED" && (
                  <div className={classes.buttons}>
                    <SecondaryButton size="medium" onClick={() => {}}>
                      Decline
                    </SecondaryButton>
                    <SecondaryButton size="medium" onClick={() => {}}>{`Accept & Sign`}</SecondaryButton>
                  </div>
                )}
              </div>
            ) : proposalsTabSelction === 0 ? (
              proposals.filter(p => p.status && p.status.toUpperCase() !== "DECLINED").length > 0 &&
              proposals
                .filter(p => p.status && p.status.toUpperCase() !== "DECLINED")
                .map((proposal, index) => (
                  <ProposalTile
                    key={`proposal-${index}`}
                    setProposal={setProposalIndexSelection}
                    index={index}
                    proposal={proposal}
                    pod={pod}
                  />
            ...
            proposals.map((proposal, index) => {
              ...
              return (
                <div className={classes.artistsList} key={`proposal-${index}`}>
                  ...
                  {proposal.artists &&
                  proposal.artists.findIndex(art => art.id === userSelector.id) !== -1 ? (
                    <>
                      {proposal.artists &&
                      proposal.artists.find(art => art.id === userSelector.id) &&
                      proposal.artists.find(art => art.id === userSelector.id).status.toUpperCase() ===
                        "PENDING" ? (
                        <div className={classes.buttons}>
                          <SecondaryButton
                            size="medium"
...
const ProposalTile = ({ proposal, pod, setProposal, index }) => {
  const classes = useStyles();
  const { convertTokenToUSD } = useTokenConversion();

  const proposer = proposal.artists.find(a => a.proposer);

  return (
    <div className={classes.proposalTile}>
      <div className={classes.proposalTileHeader}>
        <Box display={"flex"} alignItems="center">
          {proposal.date && getDate(proposal.date)}
          <span>
            {proposal.date &&
              new Date(proposal.date).getTime() > new Date().getTime() &&
              getStyledTime(new Date(), proposal.date, false)}
          </span>
        </Box>
        <h6
          onClick={() => {
      ...
      <Box display="flex" alignItems="center">
        <Box display="flex" alignItems="center" marginRight={"15px"}>
          <div
            ...
            style={{
              backgroundImage: proposer
                ? proposer.imageURL && proposer.imageURL.length > 0
                  ? `url(${proposer.imageURL})`
                  : proposer.randomAvatar
                  ? `url(${proposer.randomAvatar})`
                  : "none"
                : "none",
            }}
          />
          <Box display="flex" flexDirection="column">
            <div className={classes.artistName}>
              {proposer ? proposer.name : "Artist Name"}
              <div>Proposer</div>
            </div>
            <div className={classes.artistSlug}>
              @{proposer && proposer.slug ? proposer.slug : "Artist Name"}
            </div>
          </Box>
        </Box>
        <Box display="flex" flexDirection="column" alignItems="flex-end">
          <Box display="flex">
            <div className={classes.percent}>
              {proposer.percent ? (proposer.percent * 100).toFixed() : "N/A"}%
            </div>
            <div className={classes.price}>
              {pod.priceToken ?? "ETH"}
              {pod.price && proposer && proposer.percent ? pod.price * proposer.percent : "N/A"}
              <span>
                ($
                {pod.priceToken && pod.price && proposer && proposer.percent
                  ? convertTokenToUSD(pod.priceToken, pod.price * proposer.percent)
                  : "N/A"}
                )
              </span>
            </div>
          ...
          <div className={classes.status}>
            {proposer
              ? proposer.status.toUpperCase() === "ACCEPTED"
                ? `Accepted`
                : proposer.claimStatus.toUpperCase() === "PENDING"
                ? `Pending`
                : `Declined`
              : ""}
            <img
              src={require(`assets/icons/${
                proposer.claimStatus.toUpperCase() === "ACCEPTED"
                  ? `verified_green`
                  : proposer.claimStatus.toUpperCase() === "PENDING"
                  ? `clock_gray`
                  : `warning`
