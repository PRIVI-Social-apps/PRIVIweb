...
const CreateMediaModal = (props: any) => {
  ...
  const onMediaDataChange = key => event => {
    ...
    if (key === "type") {
      ...
      if (key === "blockchainNet" && value === BlockchainNets[2].value) {
  ...
  const handleConfirmSign = async () => {
    ...
    try {
      ...
      if (mediaData.blockchainNet === BlockchainNets[0].value) {
        ...
        const { signature } = await signPayload("createMedia", userSelector.address, newMediaData);
        const mainData: IAPIRequestProps = {
          Address: userSelector.address,
          Function: "createMedia",
          Signature: signature,
          Payload: newMediaData,
        };
        ...
        const resp = await API.createMedia(requestBody);
