...
import GiveTipModal from "components/legacy/Communities/CommunityPage/components/Jarr/modals/GiveTipModal";
...
import DiscussionSidebar from "components/legacy/Communities/CommunityPage/SideBar/DiscussionSidebar";
...
const Discord = React.memo((props: any) => {
  ...
  const [selectedDiscordSideBar, setSelectedDiscordSideBar] = useState<number>(0);
  ...
  // const selectedUpdateDiscordRoomInfo = useSelector((state: RootState) => state.updateDiscordRoomInfo);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("message-discord", message => {
        ...
        // TODO: selectedDiscordRoom is empty
        // console.log('llega aqui', selectedDiscordRoom)
  ...
  /*useEffect(() => {
        if (selectedUpdateDiscordRoomInfo
            && selectedUpdateDiscordRoomInfo.value) {
            getChatInfo(props.discordId);
            dispatch(setUpdateDiscordRoomInfo(false));
        }
    }, [selectedUpdateDiscordRoomInfo]);*/
    ...
    axios
      ...
      .then(response => {
        if (response.data.success) {
          ...
          //load images
          if (usersList && usersList.length > 0) {
            if (data.admin && usersList.some(user => user.id === data.admin.id)) {
              data.admin.imageURL =
                usersList[usersList.findIndex(user => user.id === data.admin.id)]?.imageURL ?? "";
            }
            if (data.users && data.users.length > 0) {
              data.users.forEach((member, index) => {
                if (usersList.some(user => user.id === member.id)) {
                  const user = usersList[usersList.findIndex(user => user.id === member.id)];
  ...
  const selectDiscordRoom = (room, showAll) => {
    ...
    if (showAll) {
      ...
      /*let userToAdd = autoAddUserToDiscordRoom(room);
      usersFiltered["Members"] = [...usersRoom];
      usersFiltered["Members"].push(userToAdd);*/
    } else {
      // usersFiltered["Members"] = [...usersRoom];
    }
              ...
              if (showAll) {
                ...
                if (
                  ...
                  room.users.findIndex(
                    (usr) => usr.userId === userSelector.id
                  ) !== -1*/
                ) {
                  return DisplayRooms(room, i, showAll);
                }
  ...
  const uploadPicture = () => {
    ...
    inputElement.addEventListener("change", fileInputMessagePhoto);
  ...
  const uploadAudio = () => {
    ...
    // set onchange event to call callback when user has selected file
    inputElement.addEventListener("change", fileInputMessageAudio);
          ...
          <div>
            {room.users.map((user, index) => {
              if (user.type === "Admin" && (!isDarkTheme || (isDarkTheme && index < 1))) {
                ...
                const userName = usersList[usersList.findIndex(u => u.id === user.userId)]?.name ?? "";
                return (
                  <Box
                    display="flex"
                    alignItems="center"
  ...
  return (
    <div className={`discordFullPage ${props.theme && props.theme === "dark" ? "dark" : ""}`}>
