...
export function formatNumber(number, token, numDecimals): string {
  let formattedNum = "";
  if (number >= billion)
    formattedNum = Number(parseFloat((number / billion).toFixed(numDecimals))).toString() + billionSymbol;
  else if (number >= million)
    formattedNum = Number(parseFloat((number / million).toFixed(numDecimals))).toString() + millionSymbol;
  else formattedNum = Number(parseFloat(number.toFixed(numDecimals))).toString();
...
export function generateMonthLabelsFromDate (dates:number[]) {
  ...
  const firstMonthIdx = new Date(dates[0]).getMonth();
  const lastMonthIdx = new Date(dates[dates.length-1]).getMonth();
  const numMonths = Math.abs(firstMonthIdx-lastMonthIdx);
  const pointsPerMonth = dates.length/numMonths;
  const labels:string[] = [];
  let counter = pointsPerMonth/2;
  dates.forEach(date => {
    if (counter == pointsPerMonth) {
